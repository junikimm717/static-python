...
Last updated: **2026-05-23** -- 10 benchmark reports + 1 perf follow-up,
across 5 host classes (5 x86_64 reports on 3 x86_64 hosts, 5 aarch64
reports on 2 aarch64 host classes; Neoverse-N1 has two PGO+LTO
datapoints separated by a `configure-wrapper.sh` flag tweak). Six
PGO+LTO data points span five host classes (M4 Pro, Neoverse-N1, Zen 5,
Cascade Lake, AWS EPYC) and both ISAs.
...
- **Static beats dynamic by 7-27% CPU geomean** across the PGO+LTO
  ...
  AWS EPYC 1.27x). Startup geomean remains 1-13%; Zen 5 1.01x is the
  bottom of spread because of one regressed scenario, and AWS EPYC
  1.10x joins the M4/N1/Cascade cluster.
...
- **Static wins every CPU row except `fib_recursive`.** That row is
  0.78x / 0.85x / 0.89x / 0.92x / 0.93x and is the only consistent loss
  across all five PGO+LTO host classes. N1 `perf stat` attributes it to
...
| bench            | post-PGO+LTO range  | M4 / N1 / Zen5 / Cas / EPYC | (pre-PGO range) | stresses                       |
|---               |---:                 |---:                       |---:             |---                             |
| `arith_loop`     | **1.10x - 1.52x**   | 1.14 / 1.10 / 1.52 / 1.38 / 1.51 | (1.16 - 1.45)   | bytecode int arithmetic        |
| `str_format`     | **1.11x - 1.37x**   | 1.25 / 1.11 / 1.25 / 1.20 / 1.37 | (0.97 - 1.71)   | `_string.formatter_parser`     |
| `func_call`      | **1.06x - 1.39x**   | 1.16 / 1.06 / 1.23 / 1.32 / 1.39 | (1.10 - 1.42)   | shallow Python call dispatch   |
| `fib_iter`       | **1.08x - 1.48x**   | 1.09 / 1.08 / 1.22 / 1.22 / 1.48 | (1.08 - 1.28)   | pure loop + tuple pack/unpack  |
| `attr_access`    | **1.07x - 1.24x**   | 1.17 / 1.07 / 1.22 / 1.21 / 1.22 | (1.13 - 1.23)   | LOAD_ATTR specialisation       |
| `except_path`    | **1.09x - 1.38x**   | 1.19 / 1.09 / 1.20 / 1.23 / 1.38 | (1.24 - 1.69)   | C-level exception restore      |
| `json_roundtrip` | **1.17x - 1.28x**   | 1.22 / 1.17 / 1.19 / 1.24 / 1.28 | (0.75 - 1.30)   | `_json` C decoder              |
| `listcomp`       | **1.07x - 1.27x**   | 1.27 / 1.07 / 1.18 / 1.23 / 1.20 | (1.14 - 1.26)   | list build hot path            |
| `dictops`        | **1.02x - 1.20x**   | 1.02 / 1.08 / 1.14 / 1.20 / 1.14 | (1.13 - 1.28)   | dict insert + lookup           |
| `regex_match`    | **1.03x - 1.22x**   | 1.03 / 1.07 / 1.08 / 1.22 / 1.14 | (1.14 - 1.21)   | `_sre` C extension             |

## What is mixed or wrong

| bench / scenario | M4    | N1    | Zen 5    | Cas      | EPYC     | summary (see linked report for mechanism)                                              |
|---               |---:   |---:   |---:      |---:      |---:     |---                                                                                     |
| `fib_recursive`  | 0.78x | 0.92x | 0.89x    | **0.85x**| 0.93x    | Five-host, cross-ISA static loss. N1 perf stat: branch-mispredict + backend stalls in `_PyEval_EvalFrameDefault`; NOT L1i. See [0024Z perf](./2026-05-18T0024Z_aarch64_perf.md). `-fuse-linker-plugin` did not move it. Zen 5, Cas, or EPYC perf run still owed. |
| `listcomp`       | 1.27x | 1.07x | 1.18x    | 1.23x    | 1.20x    | µarch-dependent. L1i hypothesis falsified by N1 perf stat. Spread is issue-width, not icache. See [0024Z perf](./2026-05-18T0024Z_aarch64_perf.md). |
| `stdlib` startup | 1.13x | 1.13x | **0.89x**| 1.14x    | 1.13x    | Zen-5-only -- dynamic spawns faster. Cas confirmed 1.14x across two PGO+LTO runs and EPYC landed at 1.13x; M4 + N1 + Cas + EPYC all cluster at 1.13-1.14x, so the 0.89x is almost certainly a one-time scheduling artefact on a busy local Ryzen machine. Low priority. |
